...
public class ClientAgent {
	...
	/**
	 * Sends a NetworkAction to each peer in the peer list.
	 * Does not send the action to localhost if it is an UpdateAction
	 */
	public void sendAction(NetworkAction a) throws IOException {
		for(Iterator<InetAddress> peerIterator = peers.iterator(); peerIterator.hasNext();) {
			InetAddress currentPeer = peerIterator.next();
			if(!(currentPeer.toString().equals("127.0.0.1") && a instanceof UpdateAction)) {
				Socket socket = new Socket(currentPeer, NetworkConfiguration.PORT);
	...
	/**
	 * Checks if an address is already present in the peer list.
	 * If not, the address is added to the list.
	 */
